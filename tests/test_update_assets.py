...
def compare_directories(dir1, dir2):
    ...
    for common_dir in dirs_cmp.common_dirs:
        is_identical, msg = compare_directories(
...
def test_update_assets():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        script_dir.mkdir()
        shutil.copy2(
            Path(__file__).parent.parent / "update-assets.sh",
            script_dir / "update-assets.sh"
        )
        
        # Make script executable
        os.chmod(script_dir / "update-assets.sh", 0o755)
        
        # Run the update script with custom target directory
        subprocess.run(
            ["./update-assets.sh", str(target_dir)],
            cwd=script_dir,
            check=True
        )
